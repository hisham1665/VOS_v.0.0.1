...
| Ph14 Hardening | new domain service | **Done** — kernel owns the controls it can honestly enforce; API-layer controls (transport auth, RBAC enforcement at the gateway, student data isolation, secure storage backends) remain the API layer's job (this row's intent). Delivered in the kernel: `exam/security/` (input validation on the accept path incl. traversal/extension/oversize guards, append-only atomic access log, RBAC role→action policy + `authorize`); `exam/health/` (registry availability / storage / review-queue healthcheck); `exam/provenance/` (reproducibility record: exam id/version, answer-key & rubric content versions, traced model + model version ids, capability DNA, AOS config fingerprint, timestamp, evaluation result — with a stable immutable fingerprint + identity drift verification); `exam/perf/` (graduated-scale probe 10/50/100/500/1000 through the real batch driver; failure/recovery rates, papers/sec, queue latency → `performance_results.csv`); `exam/readiness/` (executable pre-deployment checklist: no silent failures, no data loss, no auto-zero, uncertain results traceable, marks evidenced, model decisions logged, batch resumable, versions immutable). Covered by `tests/test_hardening.py` |
