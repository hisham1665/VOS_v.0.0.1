...
from aos_v0.core.capability_registry import CapabilityRegistry, InfeasibleDNAError
...
class SubAgent:
    ...
    def _route(self, node: Node) -> Tuple[object, str, List[str]]:
        ...
        if not substitutes:
            ...
            substitutes = [
                m.resource_id
                for m in self.registry.routable_manifests()
    ...
    def _route_relaxed(self, node: Node) -> Tuple[object, str, List[str]]:
        ...
        best_rid = None
        best_overlap = 0
        all_manifests = self.registry.routable_manifests()
