...
from aos_v0.core.models import CAPABILITY_FLAGS, CapabilityDNA, risk_rank
...
class CapabilityRegistry:
    ...
    def is_routable(manifest: CapabilityManifest) -> bool:
        """Whether a resource can actually execute (not a declared-only stub).

        HF registers some resources whose transports are not wired to the text
        pipeline (embeddings, rerank, image feature extraction, etc.). Their
        run_fn raises a typed error if invoked, so routing to them is a
        guaranteed runtime failure. A resource is routable unless its metadata
        explicitly marks `transport=declared`. Default-pool resources carry no
        transport marker and are therefore routable.
        """
        return manifest.metadata.get("transport", "wired") != "declared"

    def routable_manifests(self) -> List[CapabilityManifest]:
        """All registered resources that can actually execute."""
        return [
            m for m in self._manifests.values() if self.is_routable(m)
        ]
    ...
    def select(
        ...
        # Pre-pass: filter out resources that are not running AND resources that
        # are declared-only stubs (transport=declared) whose run_fn would fail
        # on invocation. Both are un-routable for different reasons.
    ...
    def feasible(
        ...
        for rid, manifest in self._manifests.items():
            if not self.is_routable(manifest):
                rejections[rid] = "declared-only interface (not wired to pipeline)"
            elif manifest.availability.status == "up":
