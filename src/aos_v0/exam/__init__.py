...
Phase 14 is production hardening of the parts the kernel can honestly own
(the heavy controls -- transport auth, RBAC enforcement, student data
isolation, secure storage backends -- are documented as the API layer's job):
`health` checks model availability, storage writability and review-queue
depth; `provenance` records the plan's reproducibility fields (exam identity
and version, answer-key/rubric content versions, traced models + model
versions, capability DNA, AOS configuration fingerprint, timestamp, evaluation
result) with a stable immutable fingerprint; `perf` probes the real batch
driver at graduated scales (failure/recovery rates, throughput, queue
latency); `readiness` executes the pre-deployment checklist (no silent
failures, no data loss, no auto-zero, traceability, evidence, logged model
decisions, resumability, immutability); `security` owns input validation,
append-only access logs and the role/action RBAC policy (`authorize`).
"""
...
from aos_v0.exam.health import HealthReport, ResourceHealth, healthcheck, render_health_table
from aos_v0.exam.perf import (
    PLAN_SCALES,
    ScaleRow,
    probe_cli_main,
    render_performance,
...
from aos_v0.exam.provenance import (
...
from aos_v0.exam.readiness import (
    ReadinessItem,
    ReadinessReport,
    readiness_cli_main,
    render_readiness,
...
from aos_v0.exam.health import (
    HealthReport,
    ResourceHealth,
    healthcheck,
    render_health_table,
