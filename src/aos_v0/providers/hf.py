...
from aos_v0.providers.errors import (
...
def validate_hf_connection(token: Optional[str]) -> str:
    """Probe whether the HF token is usable, returning WHOKAMI or None.

    Returns a short human description when the token authenticates
    ("valid — you" / "valid — <username>") and None when it is absent or
    rejected. Uses `huggingface_hub.whoami`, which makes one lightweight auth
    request and never triggers a model deployment, so startup cost is minimal.
    """
    if not token:
        return None
    try:
        ...
        return f"valid — {name}" if name else "valid"
    except Exception as exc:  # noqa: BLE001 -- any auth/network failure means unusable
        return None
