...
def build_hf_enabled_registry(
    ...
    """Default pool plus the Hugging Face catalog resources (fail-safe).
    ...
    Fail-safe: HF resources are only added if the configured token actually
    authenticates. If the token is missing or rejected (expired, invalid,
    gated), the registry degrades to the default pool -- whose resources run on
    Groq / Ollama and are unaffected by an HF token problem. This prevents the
    whole class of runtime failures where a node routes to an HF resource and
    dies on a 401, as happened in the first signed-image run.
    """
    registry = build_default_registry()

    from aos_v0.config import HF_TOKEN
    from aos_v0.providers.hf import register_hf_resources, validate_hf_connection

    token_status = validate_hf_connection(HF_TOKEN)
    if token_status is None:
        reason = (
            "missing or rejected" if HF_TOKEN else "not set"
        )
        print(
            f"\n[resource-registration] HF token {reason}; skipping "
            f"Hugging Face resources to avoid routing onto resources that "
            f"cannot authenticate. Set a valid HF_TOKEN (or HUG) to enable them."
        )
        print(
            f"[resource-registration] {len(registry.manifests())} resources "
            f"provide {len(registry.provided_flags())} distinct capability flags\n"
        )
        return registry
